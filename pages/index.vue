  ...
  export default {
    name: 'home', 
      ...
      async fetch() {
        ...
        const siteContent = await axios.get(
          `https://ddsitecontentapi-test-01.azurewebsites.net/api/v1.0/into-future/page?identifierToken=/home&locale=en-US`
        )

        // props binding to child components
        if(siteContent.data.headings){
          siteContent.data.headings.forEach((item, index) => {
            if(item.contentSlug == 'campaign-ee-launch-landing-h1'){ //Hero heading
              this.heroContent.heading = item.value;
            } else if(item.contentSlug == 'campaign-ee-launch-landing-section1-h2'){ //OfferDetails heading
              this.offerDetailsContent.heading = item.value;
            } else if(item.contentSlug == 'campaign-ee-launch-landing-section2-h2'){ //Testimonials heading
              this.testimonialsContent.heading = item.value;
            } else if(item.contentSlug == 'campaign-ee-launch-landing-section3-h2'){ //FinalSignup heading
        ...
        if(siteContent.data.textBlocks){
          siteContent.data.textBlocks.forEach((item, index) => {
            if(item.contentSlug == 'campaign-ee-launch-landing-intro'){ //Hero copyText
              this.heroContent.copyText = md.render(item.value);
            } else if(item.contentSlug == 'campaign-ee-launch-landing-section1-copy'){ //OfferDetails copyText
              this.offerDetailsContent.copyText = md.render(item.value);
            } else if(item.contentSlug == 'campaign-ee-launch-landing-section3-copy'){ //FinalSignup copyText
